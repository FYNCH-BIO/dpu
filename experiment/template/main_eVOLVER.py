...
def initialize_eVOLVER(vials, always_yes=False):
    start_time, OD_initial = eVOLVER_module.initialize_exp(vials, always_yes)
...
def custom_functions(elapsed_time):
    ...
    if OD_data == 'empty':
        ...
        elif custom_script.OPERATION_MODE == 'chemostat':
            custom_script.chemostat(OD_data, temp_data, vials, elapsed_time)
        else:
            logger.warning('unknown operation mode selected')
...
if __name__ == '__main__':
    ...
    vials = range(0, 16)
    start_time, OD_initial = initialize_eVOLVER(vials, options.always_yes)
    ...
    while True:
        try:
            # run update if at least a 100th of second has passed
            loop_time = time.time()
            if loop_time - loop_start >= 0.01:
                update_eVOLVER()
                loop_start = time.time()
