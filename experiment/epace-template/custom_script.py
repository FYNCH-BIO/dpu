...
def hybrid(eVOLVER, input_data, vials, elapsed_time):
    ...
    # arabinose_stock = 40 # X times final concentration
    # arabinose_volume = LAGOON_VOLUME / arabinose_stock # mL/hr
    # rate_arabinose = arabinose_volume / LAGOON_VOLUME # Volumes/hr
    rate_arabinose = 0.025 # Volumes/hr
    ...
    if inducer_on:
        ...
        inducer_period = (3600 * bolus_slow)/(rate_arabinose * LAGOON_VOLUME)
    else:
        inducer_period = 0
        bolus_slow_in_s = 0
...
def turbidostat(eVOLVER, input_data, vials, elapsed_time):
    ...
    lower_thresh = [0.25] * len(vials) #to set all vials to the same value, creates 16-value list
    upper_thresh = [0.35] * len(vials) #to set all vials to the same value, creates 16-value list
...
def chemostat(eVOLVER, input_data, vials, elapsed_time):
    ...
    ##### USER DEFINED VARIABLES #####
    start_OD = [-999] * 2 # ~OD600, set to 0 to start chemostate dilutions at any positive OD
    ...
    #rate_config = [0.5] * 2 #to set all vials to the same value, creates 16-value list
    rate_config = [2, 3]  #to set all vials to the same value, creates 16-value list
    rate_arabinose = 0.4
    stir = [11, 11] 
    ...
    for x in chemostat_vials: #main loop through each vial
        ...
        if data.size != 0: #waits for seven OD measurements (couple minutes) for sliding window
            ...
            # once start time has passed and culture hits start OD, if no command has been written, write new chemostat command to file
            if ((elapsed_time > start_time[x]) and (average_OD > start_OD[x]) or True):
